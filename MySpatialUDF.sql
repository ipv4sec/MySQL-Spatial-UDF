...
CREATE FUNCTION  msudf_boundary					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_buffer					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_centroid					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_convexHull				RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_contains					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_crosses					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_difference				RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_disjoint					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_intersection				RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_intersects				RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_isEmpty					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_isSimple					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_isRing					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_lineMerge				RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_lineSubstring			RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_overlaps					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_pointOnSurface			RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_reverse					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_simplify					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_simplifyPreserveTopology	RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_symDifference			RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_touches					RETURNS INTEGER SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_transform				RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_union					RETURNS STRING SONAME "libMySpatialUDF.so";
CREATE FUNCTION  msudf_within					RETURNS INTEGER SONAME "libMySpatialUDF.so";
